$(function() {
     ...
     */
    var elems = $categories2.children().length;
    var $slider2 = $('#slider2');
    /**
     * let's position all the categories on the right side of the window
     */
    var hiddenRight = $(window).width() - $categories2.offset().left;
    $categories2.children('li').css('left', hiddenRight + 'px');
    /**
     * move the first 3 categories to the viewport
     */
    $categories2.children('li:lt(3)').each(function(i) {
        var $elem = $(this);
        $elem.animate({
            'left': positions[i] + 'px',
            'opacity': 1
     ...
     * next category
     */
    $slider2.find('.next').bind('click', function() {
        if (!$categories2.children('li:nth-child(' + parseInt(first + 3) + ')').length || !navR) return;
        disableNavRight();
        disableNavLeft();
        moveRight();
    });
     ...
     */
    function moveRight() {
            var hiddenLeft = $categories2.offset().left + 340;
            var cnt = 0;
            $categories2.children('li:nth-child(' + first + ')').animate({
                'left': -hiddenLeft + 'px',
                'opacity': 0
            }, 500, function() {
                var $this = $(this);
                $categories2.children('li').slice(first, parseInt(first + 2)).each(function(i) {
                    var $elem = $(this);
                    $elem.animate({
                        'left': positions[i] + 'px'
                    }, 800, function() {
                        ++cnt;
                        if (cnt == 2) {
                            $categories2.children('li:nth-child(' + parseInt(first + 3) + ')').animate({
    ...
    * previous category
    */
    $slider2.find('.prev').bind('click', function() {
    ...
    function moveLeft() {
            ...
            var cnt = 0;
            var last = first + 2;
            $categories2.children('li:nth-child(' + last + ')').animate({
                'left': hiddenRight + 'px',
                'opacity': 0
            }, 500, function() {
                var $this = $(this);
                $categories2.children('li').slice(parseInt(last - 3), parseInt(last-1)).each(function(i) {
                    var $elem = $(this);
                    $elem.animate({
                        ...
                        ++cnt;
                        if (cnt == 2) {
                            $categories2.children('li:nth-child(' + parseInt(last - 3) + ')').animate({
                                'left': positions[0] + 'px',
                                'opacity': 1
                            }, 500, function() {
                                //$this.hide();
    ...
    */
    function disableNavRight() {
        navR = false;
        $slider2.find('.next').addClass('disabled');
    }

    function disableNavLeft() {
        navL = false;
        $slider2.find('.prev').addClass('disabled');
    }

    function enableNavRight() {
        navR = true;
        $slider2.find('.next').removeClass('disabled');
    }

    function enableNavLeft() {
        navL = true;
        $slider2.find('.prev').removeClass('disabled');
    }
});